  ...
  loadAlerts$ = this.action$.pipe(
    ...
    switchMap(() => {
      return this.alertsService.getAlerts().pipe(
        mergeMap(alerts => [
          new LoadAlertsSuccess(alerts),
          new SendSuccessMessage("Les alertes ont été chargées.")
        ]),
        catchError(error =>
          of(
            new LoadAlertsFail(error),
            new SendErrorMessage("Echec lors du chargement des alertes")
          )
        )
      );
    })
